...
class TestStackF_SDK_vs_MainAPI(unittest.TestCase):
    ...
    def test_sdk_key_used_in_main_api_encrypt(self):
        ...
        # Encrypt via main API
        r = self.client.post("/encrypt/message", json={
        ...
        enc = r.json()
        # Decrypt via main API
        r2 = self.client.post("/decrypt/message", json={
    ...
    def test_ghost_encrypt_decrypt_via_api(self):
        ...
        self.assertEqual(r.status_code, 200)
        pkg = r.json()["package"]
        r2 = self.client.post("/ghost/decrypt", json=pkg)
        self.assertEqual(r2.status_code, 200)
        self.assertEqual(r2.json()["plaintext"], "ghost connectivity test")

    def test_vault_serverless_store_retrieve(self):
        """SDK key → vault/serverless store → vault/serverless retrieve."""
        key_b64 = self.sk.new_key()
        key_hex = base64.urlsafe_b64decode(key_b64 + "==").hex()
        r_store = self.client.post("/vault/serverless", json={
            "action":  "vault_store",
            "payload": {"key_hex": key_hex, "master_password": "test-pass",
                        "n_shards": 3, "threshold": 2},
        })
        self.assertEqual(r_store.status_code, 200)
        vid = r_store.json()["vault_id"]

        r_retrieve = self.client.post("/vault/serverless", json={
            "action":  "vault_retrieve",
            "payload": {"vault_id": vid, "shard_xs": [1, 2],
                        "master_password": "test-pass"},
        })
        self.assertEqual(r_retrieve.status_code, 200)
        self.assertEqual(r_retrieve.json()["key_hex"], key_hex)
...
class TestStackG_SDK_vs_LightweightServer(unittest.TestCase):
    ...
    def test_new_key(self):
        r = self.client.post("/key/new", json={})
        self.assertEqual(r.status_code, 200)
        d = r.json()
    ...
    def test_new_key_with_passphrase(self):
        r = self.client.post("/key/new", json={"passphrase": "my-secret"})
        self.assertEqual(r.status_code, 200)
        self.assertIn("key_b64", r.json())

    def test_encrypt_decrypt_text(self):
        key = self.client.post("/key/new", json={}).json()["key_b64"]
        r_enc = self.client.post("/encrypt", json={
            "plaintext": "lightweight server test",
            "key_b64":   key,
            "context":   "server-test",
        })
        self.assertEqual(r_enc.status_code, 200)
        envelope = r_enc.json()["envelope"]

        r_dec = self.client.post("/decrypt", json={"envelope": envelope, "key_b64": key})
        self.assertEqual(r_dec.status_code, 200)
        self.assertEqual(r_dec.json()["plaintext"], "lightweight server test")

    def test_encrypt_decrypt_file(self):
        key       = self.client.post("/key/new", json={}).json()["key_b64"]
        # Use non-UTF-8 bytes so the server returns binary/data_b64 path
        file_data = bytes(range(0, 256))
        r_enc = self.client.post("/encrypt-file", json={
    ...
    def test_wrong_key_returns_400(self):
        key   = self.client.post("/key/new", json={}).json()["key_b64"]
        wrong = self.client.post("/key/new", json={}).json()["key_b64"]
        env   = self.client.post("/encrypt", json={"plaintext": "x", "key_b64": key}).json()["envelope"]
        r     = self.client.post("/decrypt", json={"envelope": env, "key_b64": wrong})
    ...
    def test_sdk_envelope_decrypted_by_server(self):
        ...
        r   = self.client.post("/decrypt", json={"envelope": env, "key_b64": key})
        self.assertEqual(r.status_code, 200)
        self.assertEqual(r.json()["plaintext"], "cross-layer test")
...
class TestStackX_CrossStackCompatibility(unittest.TestCase):
    ...
    def test_sdk_server_key_in_sdk_core(self):
        ...
        key_b64 = client.post("/key/new", json={}).json()["key_b64"]
        sk      = SumitKey()
        env     = sk.encrypt_text("server-core bridge", key_b64)
        r       = client.post("/decrypt", json={"envelope": env, "key_b64": key_b64})
        self.assertEqual(r.json()["plaintext"], "server-core bridge")
